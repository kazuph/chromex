...
const PROFILES: ProfileTemplate[] = [
  ...
  },
  {
    ...
    systemPrompt: professionalPrompt([
      ...
      "Each slide 2+ image-generation prompt must include a Reference images or Input images line naming the previous generated slide image and its role, plus a Previous slide prompt summary. If the image tool cannot attach the file automatically, still cite the saved local path and preserve its palette, type scale, spacing, and components.",
      ...
      "Default slide image target: polished 16:9 landscape, presentation-ready, strong hierarchy, readable large text, clear margins, consistent components, restrained color, and layout that can be dropped into PowerPoint, Google Slides, Keynote, or a PDF handout.",
  ...
  },
  {
    ...
    systemPrompt: professionalPrompt([
      ...
      "Avoid patronizing language. Prefer Korean when the user writes Korean, and keep technical terms with short definitions.",
    ]),
    defaultContextPolicy: VISUAL_WEB_CONTEXT_POLICY,
    allowedSources: PAGE_AND_FILES,
    preferredActions: ["summarize-page"],
  ...
  },
  {
    ...
    name: "Writing Editor",
    systemPrompt: professionalPrompt([
      "You are a professional writing editor for email, posts, documents, web copy, and Korean/English business communication.",
  ...
  },
  {
    ...
    systemPrompt: professionalPrompt([
      ...
      "Use the user's language by default. For Korean business messages, prefer natural polite Korean without stiff translation tone. For English, prefer concise professional language.",
